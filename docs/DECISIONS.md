...
## ADR-011. AI Caddie Data Binding은 Engine과 분리한다

### 결정

AI Caddie의 DB 조회와 계산 엔진은 분리한다.
...
- AI Engine은 순수 함수로 유지해야 테스트와 개선이 쉽다.
- Supabase 조회 실패가 Home 전체 실패로 번지지 않도록 한다.
- DB Row 구조가 바뀌어도 Engine 입력 모델은 안정적으로 유지한다.

### 규칙

- Engine은 Supabase를 import하지 않는다.
- UI 컴포넌트는 거리 계산, 클럽 추천, 위험도 분석을 직접 수행하지 않는다.
- `course_hole_guides`, `user_distance_profiles`, `user_preferences`는 Mapper를 거쳐 Engine 입력으로 변환한다.
