...
export async function getRounds(clubId: string): Promise<SavedRound[]> {
  const { data, error } = await supabase
    .from('rounds')
    .select('id, date, course_name, pars, shinperio_holes, players')
    .eq('club_id', clubId)
...
export async function updateRound(
  id: string,
  input: { courseName: string; pars: number[]; players: PlayerScore[]; date?: string; photoData?: string[] }
): Promise<SavedRound> {
  const payload: Record<string, unknown> = {
    course_name: input.courseName || '이름 없는 코스',
    pars: input.pars,
...
export function computeHandicaps(rounds: SavedRound[]): Map<string, number> {
  ...
  for (const [name, entries] of byPlayer) {
    const last5 = [...entries].sort((a, b) => a.date.localeCompare(b.date)).slice(-5)
    result.set(name, Math.ceil(last5.reduce((s, e) => s + e.diff, 0) / last5.length))
  }
  return result
}
