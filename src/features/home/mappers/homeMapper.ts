...
function mapScheduleRound(raw: HomeDashboardRawData, schedule: HomeScheduleRow, userId?: string | null): HomeHeroRound {
  ...
  return {
    ...
    openWeatherHours: weather?.openWeatherHours,
    ...
    clubhouseLatitude: course?.clubhouse_latitude ?? null,
    clubhouseLongitude: course?.clubhouse_longitude ?? null,
    navigationName: course?.navigation_name ?? null,
    navigationAddress: course?.navigation_address ?? null,
  ...
  const items: PremiumRecentStatItem[] = [
    {
      ...
      caption: myTotals.length ? '최근 5경기' : '기록 등록 필요',
      tone: 'primary',
    ...
    },
    {
      ...
      caption: myTotals.length ? '최근 기록 기준' : '첫 라운드를 기록하세요',
      tone: 'info',
    ...
    },
    {
      ...
      caption: myTotals.length ? '최근 기록 기준' : '도전 기록 대기',
      tone: 'gold',
