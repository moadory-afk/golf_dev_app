...
function getWinnerLocal(
  r: SavedRound,
  handicaps: Map<string, number>,
): string | null {
  const best = Math.min(...r.players.map((p) => playerTotal(p.strokes)));
  const medalWinner = r.players.find(
    (p) => playerTotal(p.strokes) === best,
  )?.name;
  const ranked = r.players
    .map((p) => ({
      name: p.name,
      net: playerTotal(p.strokes) - (handicaps.get(p.name) ?? 0),
    }))
...
function getPlayerBadges(
  ...
  for (const r of sorted) {
    ...
    if (w && w === curPlayer) {
      ...
      if (curPlayer && curStreak > 0)
        streakMap.set(
          curPlayer,
          Math.max(streakMap.get(curPlayer) ?? 0, curStreak),
        );
      curPlayer = w ?? "";
      curStreak = w ? 1 : 0;
    }
  ...
  let maxStreak = 0,
    maxStreakPlayer = "";
  for (const [n, st] of streakMap)
    if (st > maxStreak) {
      maxStreak = st;
      maxStreakPlayer = n;
    }
  if (maxStreak >= 2 && maxStreakPlayer)
    add(maxStreakPlayer, "🔥", `${maxStreak}연승`);
  ...
  let topSingleName = "",
    topSingleCount = 0;
  for (const [n, c] of singleBirdie)
    if (c > topSingleCount) {
      topSingleCount = c;
      topSingleName = n;
    }
  if (topSingleName && topSingleCount > 0)
    add(topSingleName, "⛳", "한경기버디");
    ...
    <View style={{ flex: 1, backgroundColor: C.bg }}>
      ...
      >
        {!clubsLoaded || loading ? (
          ...
          <>
            {tab === "byRound" && (
              <ByRound rounds={rounds} handicapBasis={handicapBasis} />
            )}
            {tab === "byPlayer" && (
              <ByPlayer
                rounds={rounds}
            ...
            {tab === "club" && (
              <Club rounds={rounds} handicapBasis={handicapBasis} />
            )}
            {tab === "hall" && (
              <HallOfFame rounds={rounds} handicapBasis={handicapBasis} />
            )}
          </>
        )}
      </ScrollView>
...
function ByRound({
  ...
  if (rounds.length === 0)
    return <Text style={s.muted}>아직 라운드 기록이 없습니다.</Text>;

  const months = [...new Set(rounds.map((r) => r.date.slice(0, 7)))].sort(
    ...
    <>
      ...
      {filtered.length === 0 ? (
        ...
        filtered.map((r) => {
          ...
          const bestPlayer = r.players.find(
            (p) => playerTotal(p.strokes) === best,
          );
          ...
          if (priorRounds.length > 0) {
            ...
            const priorBest = Math.min(
              ...priorRounds.flatMap((pr) =>
                pr.players.map((p) => playerTotal(p.strokes)),
              ),
            );
            if (best < priorBest)
              newRecords.push({
                icon: "🏆",
                text: `최저타 갱신 ${bestPlayer ? shortName(bestPlayer.name) : ""} ${best}타`,
              });

            // 버디왕 갱신
            const priorMaxBirdie = Math.max(
              0,
              ...priorRounds.flatMap((pr) =>
                pr.players.map((p) => {
                  let b = 0;
            ...
            if (birdieTop && birdieTop.count > priorMaxBirdie)
              newRecords.push({
                icon: "🐦",
                text: `버디왕 갱신 ${shortName(birdieTop.name)} ${birdieTop.count}개`,
              });

            // 파왕 갱신
            const priorMaxPar = Math.max(
              0,
              ...priorRounds.flatMap((pr) =>
                pr.players.map((p) => {
                  let cnt = 0;
                  p.strokes.forEach((s, i) => {
                    if (s - pr.pars[i] === 0) cnt++;
                  });
                  return cnt;
                }),
              ),
            ...
            if (parTop && parTop.count > priorMaxPar)
              newRecords.push({
                icon: "⛳",
                text: `파왕 갱신 ${shortName(parTop.name)} ${parTop.count}개`,
              });

            // 최다우승 갱신
            const winsMap = new Map<string, number>();
            for (const pr of priorRounds) {
              const w = getWinnerLocal(
                pr,
                getHandicapsForRound(pr, rounds, handicapBasis),
              );
            ...
            if (thisWinner) {
              ...
              if (newWins > priorMaxWins)
                newRecords.push({
                  icon: "🥇",
                  text: `최다우승 갱신 ${shortName(thisWinner)} ${newWins}회`,
                });
            }

            // 최저핸디 갱신: 클럽 역대 최저 핸디 갱신
            const priorHandiVals = priorRounds
              .flatMap((pr) =>
                pr.players.map((p) => {
                  const rel = priorRounds
                    .filter(
                      (x) =>
                        x.date <= pr.date &&
                        x.players.some((pl) => pl.name === p.name),
                    )
              ...
              .filter((h) => isFinite(h));
            const prevMinH = priorHandiVals.length
              ? Math.min(...priorHandiVals)
              : Infinity;
            ...
            if (finiteHandis.length > 0 && isFinite(prevMinH)) {
              ...
              if (curMinH < prevMinH) {
                ...
                newRecords.push({
                  icon: "📉",
                  text: `최저핸디 갱신 ${shortName(top.name)} ${top.h > 0 ? "+" : ""}${top.h}`,
                });
              }
            }
          }
              ...
              >
                {bestPlayer && (
                  <View style={[s.statChip, { backgroundColor: "#fffbe8" }]}>
                    ...
                    <Text style={s.statChipText}>
                      {shortName(bestPlayer.name)}{" "}
                      <Text style={{ color: C.gold }}>{best}</Text>
                    </Text>
                  </View>
                )}
                {winner && (
                  <View style={[s.statChip, { backgroundColor: C.greenLight }]}>
                    ...
                    <Text style={s.statChipText}>
                      {shortName(winner.name)}{" "}
                      <Text style={{ color: C.green }}>
                        {diffText(winner.netVsPar)}
                      </Text>
                    </Text>
                  </View>
                )}
                {runnerUp && (
                  <View style={[s.statChip, { backgroundColor: "#f4f6f8" }]}>
                    <EmojiIcon char="🥈" size={12} />
                    <Text style={s.statChipText}>
                      {shortName(runnerUp.name)}{" "}
                      <Text style={{ color: C.muted }}>
                        {diffText(runnerUp.netVsPar)}
                      </Text>
                    </Text>
                  </View>
                )}
                ...
                {newRecords.length > 0 ? (
                  ...
                  <>
                    {birdieTop && birdieTop.count > 0 && (
                      <View
                        style={[s.statChip, { backgroundColor: "#eff6ff" }]}
                      >
                        <EmojiIcon char="🐦" size={12} color={C.info} />
                        <Text style={s.statChipText}>
                          {shortName(birdieTop.name)}{" "}
                          <Text style={{ color: C.info }}>
                            {birdieTop.count}개
                          </Text>
                        </Text>
                      </View>
                    ...
                    {parTop && parTop.count > 0 && (
                      <View
                        style={[s.statChip, { backgroundColor: "#f0f5f2" }]}
                      >
                        <EmojiIcon char="⛳" size={12} color={C.green} />
                        <Text style={s.statChipText}>
                          {shortName(parTop.name)}{" "}
                          <Text style={{ color: C.green }}>
                            {parTop.count}개
                          </Text>
                        </Text>
...
function ByPlayer({
  ...
  const [detailModal, setDetailModal] = useState<
    ...
    | null
  >(null);
  const [personalStatsBySchedule, setPersonalStatsBySchedule] = useState<
    Record<string, PersonalRoundHoleStat[]>
  >({});
  ...
  const playerRounds = myName
    ? [...(byName.get(myName) ?? [])].sort((a, b) =>
        b.date.localeCompare(a.date),
      )
    : [];
  const scheduleIds = playerRounds
    .map(
      (round) => rounds.find((item) => item.id === round.roundId)?.scheduleId,
    )
  ...
  if (!myName || playerRounds.length === 0)
    return <Text style={s.muted}>내 개인 기록 데이터가 없습니다.</Text>;
  ...
  const best = Math.min(...totals);
  const lastN = [...playerRounds]
    .sort((a, b) => a.date.localeCompare(b.date))
    .slice(-handicapBasis);
  const handicap = Math.ceil(
    lastN.reduce((sum, round) => sum + round.diff, 0) / lastN.length,
  );
  const recent5 = playerRounds.slice(0, 5);
  const recent5Avg = Math.ceil(
    recent5.reduce((sum, round) => sum + round.total, 0) / recent5.length,
  );
  const oldestRecent = recent5[recent5.length - 1];
  const latestRecent = recent5[0];
  const trendText =
    oldestRecent && latestRecent
  ...
  const avgParType = (par: 3 | 4 | 5) =>
    parType[par].count
      ? (parType[par].total / parType[par].count).toFixed(1)
      : "-";
  ...
  const playerStats = [...byName.entries()].map(([name, list]) => {
    ...
    return {
      name,
      avg: Math.ceil(
        playerTotals.reduce((sum, total) => sum + total, 0) /
          playerTotals.length,
      ),
      handicap: Math.ceil(
        playerLastN.reduce((sum, round) => sum + round.diff, 0) /
          playerLastN.length,
      ),
  ...
  const rankOf = (
    items: typeof playerStats,
    key: "avg" | "handicap" | "birdie",
    lowerBetter: boolean,
  ) => {
    const sorted = [...items].sort((a, b) =>
      lowerBetter ? a[key] - b[key] : b[key] - a[key],
    );
  ...
  const personalHoleStats = scheduleIds.flatMap(
    (scheduleId) => personalStatsBySchedule[scheduleId] ?? [],
  );
  const personalHoleStatsWithScore = scheduleIds.flatMap((scheduleId) => {
    ...
    const playerRound = round
      ? playerRounds.find((item) => item.roundId === round.id)
      : null;
  ...
  const firSuccess = firTargets.filter(
    (item) => !item.fir || item.fir === "center",
  ).length;
  const firRate = firTargets.length
    ? Math.round((firSuccess / firTargets.length) * 100)
    : null;
  const girTargets = personalHoleStatsWithScore.filter(
    (item) => item.score !== null && item.putts > 0,
  );
  const girSuccess = girTargets.filter(
    (item) => item.score !== null && item.score - item.putts <= item.par - 2,
  ).length;
  const girRate = girTargets.length
    ? Math.round((girSuccess / girTargets.length) * 100)
    : null;
  const obCount = personalHoleStats.filter(
    (item) =>
      item.fir === "left_ob" ||
      item.fir === "right_ob" ||
      item.fir === "other_ob",
  ).length;
  const hazardCount = personalHoleStats.filter(
    (item) => item.fir === "hazard",
  ).length;
  const avgPutts = personalHoleStats.length
    ? (
        personalHoleStats.reduce((sum, item) => sum + item.putts, 0) /
        personalHoleStats.length
      ).toFixed(1)
    : "-";
  const threePuttCount = personalHoleStats.filter(
    (item) => item.putts >= 3,
  ).length;
  const penaltyTotal = personalHoleStats.reduce(
    (sum, item) => sum + item.penalties,
    0,
  );
  const firCounts = new Map<PersonalRoundFir, number>();
  for (const item of personalHoleStats)
    if (item.fir) firCounts.set(item.fir, (firCounts.get(item.fir) ?? 0) + 1);
  const mainMiss = [...firCounts.entries()]
    .filter(([key]) => key !== "center")
    .sort((a, b) => b[1] - a[1])[0];
  const mainMissText = mainMiss ? firLabel(mainMiss[0]) : "미입력";
  const roundsOldToNew = [...playerRounds].sort((a, b) =>
    a.date.localeCompare(b.date),
  );
  const trendRounds = roundsOldToNew.slice(-6);
  const trendWithStats = trendRounds.map((round) => {
    const scheduleId = rounds.find(
      (item) => item.id === round.roundId,
    )?.scheduleId;
    const holeStats = scheduleId
      ? (personalStatsBySchedule[scheduleId] ?? [])
      : [];
    const holeStatsWithScore = holeStats.map((item) => ({
      ...item,
      score: round.strokes[item.hole - 1] ?? null,
    }));
    const roundFirTargets = holeStats.filter((item) => item.par !== 3);
    const roundGirTargets = holeStatsWithScore.filter(
      (item) => item.score !== null && item.putts > 0,
    );
    return {
      round,
      fir: roundFirTargets.length
        ? Math.round(
            (roundFirTargets.filter(
              (item) => !item.fir || item.fir === "center",
            ).length /
              roundFirTargets.length) *
              100,
          )
        : null,
      gir: roundGirTargets.length
        ? Math.round(
            (roundGirTargets.filter(
              (item) =>
                item.score !== null && item.score - item.putts <= item.par - 2,
            ).length /
              roundGirTargets.length) *
              100,
          )
        : null,
      putts: holeStats.length
        ? Number(
            (
              holeStats.reduce((sum, item) => sum + item.putts, 0) /
              holeStats.length
            ).toFixed(1),
          )
        : null,
  ...
  const toTrendData = <T extends { round: PlayerRound }>(
    items: T[],
  ...
  const scoreDistributionData = [
    ...
    {
      label: "더블+",
      value: scoreTotals.double + scoreTotals.triplePlus,
      color: C.danger,
    },
  ];
  ...
  const personalReportCards: {
    key: NonNullable<typeof detailModal>;
    icon: string;
    title: string;
    subtitle: string;
  }[] = [
    {
      ...
      subtitle: target
        ? `${target}타 목표 관리`
        : `${Math.max(1, avg - 3)}타 목표 추천`,
    },
    {
      key: "trend",
      icon: "📈",
      title: "스코어 추이",
      subtitle: `최근5 평균 ${recent5Avg}타`,
    },
    {
      key: "shot",
      icon: "⛳",
      title: "샷·퍼팅",
      subtitle: `FIR ${firRate === null ? "-" : `${firRate}%`} · 퍼팅 ${avgPutts}`,
    },
    {
      key: "hole",
      icon: "🏌️",
      title: "홀 유형",
      subtitle: weakness ? `${weakness.label} 보완 필요` : "Par별 평균 분석",
    },
    {
      key: "score",
      icon: "🥧",
      title: "스코어 분포",
      subtitle: `파 ${scoreTotals.par} · 보기 ${scoreTotals.bogey}`,
    },
    {
      key: "rank",
      icon: "🏅",
      title: "클럽 순위",
      subtitle: `${totalPlayers}명 중 내 위치`,
    },
    {
      key: "rounds",
      icon: "📋",
      title: "라운드 상세",
      subtitle: `${playerRounds.length}개 라운드 기록`,
    },
    {
      key: "improve",
      icon: "💡",
      title: "개선 리포트",
      subtitle: weakness ? `${weakness.label} 집중 공략` : "AI 개선 포인트",
    },
  ];
    ...
    <>
              ...
              <ScrollView showsVerticalScrollIndicator={false}>
                  ...
                  <>
                    <View style={s.goalRow}>
                      <TextInput
                        ...
                        value={targetScore}
                        onChangeText={(value) =>
                          setTargetScore(value.replace(/[^0-9]/g, ""))
                        }
                    ...
                    <Text style={s.insightText}>
                      {target
                        ? targetGap > 0
                          ? `현재 평균에서 ${targetGap}타를 줄이면 목표에 도달합니다.`
                          : "현재 평균이 목표 수준에 도달했습니다."
                        : "목표 타수를 입력하면 현재 평균과 비교합니다."}
                    </Text>
                  </>
                )}
                {detailModal === "trend" && (
                  <>
                            ...
                            <View
                              style={[
                                s.miniTrendBar,
                                {
                                  height: Math.max(
                                    18,
                                    72 - (round.total - best) * 2,
                                  ),
                                },
                              ]}
                            />
                            <Text style={s.miniTrendDate}>
                              {round.date.slice(5)}
                            </Text>
                          </View>
                        ))}
                    ...
                    <View style={s.analysisRow}>
                      <Text style={s.analysisLabel}>최근 5경기 평균</Text>
                      <Text style={s.analysisValue}>{recent5Avg}타</Text>
                    </View>
                    <View style={s.analysisRow}>
                      <Text style={s.analysisLabel}>전후반 차이</Text>
                      <Text style={s.analysisValue}>
                        {diffText(backAvg - frontAvg)}타
                      </Text>
                    </View>
                  </>
                )}
                {detailModal === "hole" && (
                  <>
                    ...
                    {strength && weakness && (
                      <Text style={s.insightText}>
                        강점은 {strength.label}, 보완 포인트는 {weakness.label}
                        입니다.
                      </Text>
                    )}
                  </>
                ...
                {detailModal === "score" && (
                  <>
                    ...
                    <View style={s.scoreDistRow}>
                      <ScoreDist
                        label="버디"
                        value={scoreTotals.birdie}
                        color={C.info}
                      />
                      <ScoreDist
                        label="파"
                        value={scoreTotals.par}
                        color={C.green}
                      />
                      <ScoreDist
                        label="보기"
                        value={scoreTotals.bogey}
                        color={C.warn}
                      />
                      <ScoreDist
                        label="더블"
                        value={scoreTotals.double}
                        color={C.danger}
                      />
                      <ScoreDist
                        label="트리플+"
                        value={scoreTotals.triplePlus}
                        color={C.text}
                      />
                    </View>
                  </>
                ...
                {detailModal === "rank" && (
                  <>
                    <View style={s.analysisRow}>
                      <Text style={s.analysisLabel}>평균 순위</Text>
                      <Text style={s.analysisValue}>
                        {rankOf(playerStats, "avg", true)} / {totalPlayers}
                      </Text>
                    </View>
                    <View style={s.analysisRow}>
                      <Text style={s.analysisLabel}>핸디 순위</Text>
                      <Text style={s.analysisValue}>
                        {rankOf(playerStats, "handicap", true)} / {totalPlayers}
                      </Text>
                    </View>
                    <View style={s.analysisRow}>
                      <Text style={s.analysisLabel}>버디 순위</Text>
                      <Text style={s.analysisValue}>
                        {rankOf(playerStats, "birdie", false)} / {totalPlayers}
                      </Text>
                    </View>
                  </>
                )}
                {detailModal === "shot" && (
                  <>
                    ...
                    <View style={s.metricGrid}>
                      <MetricCard
                        label="FIR 성공률"
                        value={firRate === null ? "-" : `${firRate}%`}
                      />
                      <MetricCard
                        label="GIR 성공률"
                        value={girRate === null ? "-" : `${girRate}%`}
                      />
                      <MetricCard
                        label="평균 퍼팅"
                        value={avgPutts === "-" ? "-" : `${avgPutts}개`}
                      />
                      <MetricCard
                        label="OB/해저드"
                        value={`${obCount}/${hazardCount}`}
                      />
                      <MetricCard label="패널티" value={`${penaltyTotal}개`} />
                    </View>
                    <View style={s.analysisRow}>
                      <Text style={s.analysisLabel}>주요 미스</Text>
                      <Text style={s.analysisValue}>{mainMissText}</Text>
                    </View>
                    <View style={s.analysisRow}>
                      <Text style={s.analysisLabel}>3퍼트 이상</Text>
                      <Text style={s.analysisValue}>{threePuttCount}회</Text>
                    </View>
                    <Text style={s.insightText}>
                      OB와 해저드는 티샷 리스크, 3퍼트는 그린 위 손실로 나눠
                      관리하면 개선 포인트가 더 선명해집니다.
                    </Text>
                  </>
                )}
                {detailModal === "improve" &&
                    ...
                    <View>
                        ...
                        {[
                          ...
                          <Text
                            ...
                            style={[
                              s.th,
                              {
                                width: [54, 82, 42, 52, 42, 42, 92][index],
                      ...
                      {playerRounds.map((round) => (
                        <View key={round.roundId} style={s.tableRow}>
                          <Text style={[s.td, { width: 54 }]}>
                            {round.date.slice(5)}
                          </Text>
                          <Text style={[s.td, { width: 82 }]} numberOfLines={1}>
                            {round.courseName.slice(0, 7)}
                          </Text>
                          <Text
                            style={[
                              s.td,
                          ...
                          <Text
                            style={[
                              s.td,
                              {
                                width: 52,
                                textAlign: "right",
                                color: round.diff <= 0 ? C.green : C.warn,
                              },
                            ]}
                          >
                            {diffText(round.diff)}
                          ...
                          <Text
                            style={[s.td, { width: 42, textAlign: "right" }]}
                          >
                            {round.front}
                          </Text>
                          <Text
                          ...
                          <Text
                            style={[s.td, { width: 92, textAlign: "right" }]}
                          >
                            {round.birdie}/{round.parCount}/{round.bogey}/
                            {round.double}/{round.triplePlus}
                          </Text>
                        </View>
                      ))}
                    </View>
      ...
      <View style={s.aiCaddieCard}>
        ...
        <View style={s.aiCaddieRecommendRow}>
          ...
          <Text style={s.aiCaddieRecommendText} numberOfLines={2}>
            {weakness
              ? `${weakness.label} 공략을 안정적으로 가져가면 평균 스코어 개선 여지가 큽니다.`
              : "최근 라운드 패턴을 더 쌓으면 맞춤 개선 포인트를 제안할게요."}
          </Text>
        </View>
      </View>
      ...
      <View style={s.personalReportSection}>
        <View style={s.personalReportHeader}>
          ...
          <Text style={s.personalReportHint}>좌우로 넘겨 보기</Text>
        </View>
        <ScrollView
          horizontal
          ...
          contentContainerStyle={s.personalReportScroller}
        >
          {personalReportCards.map((card) => (
            <PersonalReportCard
              ...
              onPress={() => setDetailModal(card.key)}
            />
          ))}
        </ScrollView>
      </View>
...
function MetricCard({
  ...
  return (
    <View style={s.metricCard}>
      ...
      <Text style={[s.metricValue, tone ? { color: tone } : null]}>
        {value}
      </Text>
    </View>
  );
...
function PersonalReportCard({
  ...
  return (
    <TouchableOpacity
      style={s.personalReportCard}
      activeOpacity={0.86}
      onPress={onPress}
    >
      <Text style={s.personalReportIcon}>{icon}</Text>
      <Text style={s.personalReportCardTitle} numberOfLines={1}>
        {title}
      </Text>
      <Text style={s.personalReportCardSubtitle} numberOfLines={2}>
        {subtitle}
      </Text>
      <Text style={s.personalReportArrow}>›</Text>
    </TouchableOpacity>
  );
}
  ...
  return (
    <View style={s.visualCard}>
      <View style={s.visualHeader}>
        <Text style={s.visualTitle}>퍼팅 추세</Text>
        <Text style={s.visualValue}>
          {data.length ? `${data[data.length - 1].value}개` : "-"}
        </Text>
      </View>
      <View style={s.puttBarRow}>
        {data.length === 0 ? (
          <Text style={s.visualEmpty}>추세 데이터가 없습니다.</Text>
        ) : (
          data.map((item) => (
            <View key={item.date} style={s.puttBarItem}>
              ...
              <View
                style={[
                  s.puttBar,
                  { height: Math.max(12, (item.value / max) * 58) },
                ]}
              />
...
function ObDistribution({
  ...
  return (
    <View style={s.visualCard}>
      <View style={s.visualHeader}>
        <Text style={s.visualTitle}>OB/해저드 분포</Text>
        <Text style={s.visualValue}>
          {data.reduce((sum, item) => sum + item.value, 0)}회
        </Text>
      </View>
      <View style={s.obGrid}>
        {data.map((item) => (
...
function RadarChart({ data }: { data: { label: string; value: number }[] }) {
  ...
  const normalized = (value: number) =>
    max === min ? 0.72 : 0.35 + ((max - value) / (max - min)) * 0.5;
  ...
  const outerPoints = data
    .map((_, index) => point(index, 1))
    .map((p) => `${p.x},${p.y}`)
    .join(" ");
  const valuePoints = data
    .map((item, index) => point(index, normalized(item.value)))
    .map((p) => `${p.x},${p.y}`)
    .join(" ");
  return (
    <View style={s.visualCard}>
      <View style={s.visualHeader}>
        <Text style={s.visualTitle}>홀 유형 밸런스</Text>
        <Text style={s.visualValue}>낮을수록 강점</Text>
      </View>
      {data.length < 3 ? (
        <Text style={s.visualEmpty}>분석 데이터가 부족합니다.</Text>
      ) : (
        <Svg width={size} height={size}>
          <Polygon
            points={outerPoints}
...
function ScoreDonut({
  ...
  return (
    <View style={s.visualCard}>
      <View style={s.visualHeader}>
        <Text style={s.visualTitle}>스코어 구성</Text>
        <Text style={s.visualValue}>{total}홀</Text>
      </View>
      {total === 0 ? (
        <Text style={s.visualEmpty}>스코어 데이터가 없습니다.</Text>
      ) : (
...
function StackedScoreBars({
  ...
  return (
    <View style={s.visualCard}>
      <View style={s.visualHeader}>
        <Text style={s.visualTitle}>최근 라운드 구성</Text>
        <Text style={s.visualValue}>스택</Text>
      </View>
      {data.length === 0 ? (
        <Text style={s.visualEmpty}>추세 데이터가 없습니다.</Text>
      ) : (
        data.map((item) => {
          const total = Math.max(
            1,
            item.birdie + item.par + item.bogey + item.doublePlus,
          );
          return (
            <View key={item.date} style={s.stackRow}>
              ...
              <View style={s.stackTrack}>
                <View
                  style={[
                    s.stackSeg,
                    { flex: item.birdie, backgroundColor: C.info },
                  ]}
                />
                <View
                  style={[
                    s.stackSeg,
                    { flex: item.par, backgroundColor: C.green },
                  ]}
                />
                <View
                  style={[
                    s.stackSeg,
                    { flex: item.bogey, backgroundColor: C.warn },
                  ]}
                />
                <View
                  style={[
                    s.stackSeg,
                    { flex: item.doublePlus, backgroundColor: C.danger },
                  ]}
                />
                {total === 1 &&
                  item.birdie + item.par + item.bogey + item.doublePlus ===
                    ...
                    <View
                      style={[
                        s.stackSeg,
                        { flex: 1, backgroundColor: C.border },
                      ]}
                    />
                  )}
              </View>
...
function Club({
  rounds,
  handicapBasis: currentHandicapBasis,
}: {
  rounds: SavedRound[];
  handicapBasis: HandicapBasis;
}) {
  const [showChart, setShowChart] = useState<"avg" | "best" | false>(false);
  const [handicapBasis, setHandicapBasis] =
    useState<HandicapBasis>(currentHandicapBasis);
  ...
  if (rounds.length === 0)
    return <Text style={s.muted}>데이터가 없습니다.</Text>;
  ...
  const byName = new Map<
    string,
    Array<{ date: string; total: number; par: number }>
  >();
  for (const r of rounds) {
    const par = totalPar(r.pars);
    for (const p of r.players) {
  ...
  const stats = Array.from(byName.entries())
    .map(([name, entries]) => {
      ...
      const handicap = Math.ceil(
        lastN.reduce((sum, e) => sum + (e.total - e.par), 0) / lastN.length,
      );
      return {
        name,
  ...
  const roundAvgs = [...rounds]
    ...
    .map((r) => ({
      date: r.date,
      value: Math.ceil(
        r.players.reduce((sum, p) => sum + playerTotal(p.strokes), 0) /
          r.players.length,
      ),
    }));
                  ...
                  {([3, 5, 10] as const).map((n) => (
                    ...
                    >
                      <Text
                        style={[
                          s.dropdownItemText,
                          handicapBasis === n && s.dropdownItemActive,
                        ]}
                      >
        ...
        {stats.map((stat, i) => {
          ...
          return (
            ...
            >
              ...
              <Text
                style={[s.td, { flex: 2, fontWeight: i < 3 ? "700" : "400" }]}
              >
                {shortName(stat.name)}
              </Text>
...
function HallOfFame({
  ...
  if (rounds.length === 0)
    return <Text style={s.muted}>기네스 북 데이터가 없습니다.</Text>;
  ...
  for (const r of rounds) {
    for (const p of r.players) {
      ...
      if (!prevPar || parCount > prevPar.count)
        singleParMap.set(p.name, {
          count: parCount,
          date: r.date,
          courseName: r.courseName,
        });
  ...
  const singleBirdieRanking = [...singleBirdieMap.entries()]
    ...
    .filter((x) => x.count > 0)
    .sort((a, b) => b.count - a.count);
  const singleParRanking = [...singleParMap.entries()]
    .map(([name, v]) => ({ name, ...v }))
    .filter((x) => x.count > 0)
    .sort((a, b) => b.count - a.count);
  const handicapRanking = [...handicaps.entries()]
    .sort((a, b) => a[1] - b[1])
    .map(([name, handicap]) => ({ name, handicap }));
  const lowestScoreRanking = [...scoreRecords].sort(
    (a, b) => a.total - b.total,
  );
  const highestScoreRanking = [...scoreRecords].sort(
    (a, b) => b.total - a.total,
  );

  const playerRounds = new Map<
    string,
    { date: string; total: number; diff: number }[]
  >();
  const frontBackRanking: {
    name: string;
    improvement: number;
    front: number;
    back: number;
  ...
  for (const r of rounds) {
    ...
    for (const p of r.players) {
      ...
      const front = p.strokes
        .slice(0, 9)
        .reduce((sum, score) => sum + score, 0);
      const back = p.strokes
        .slice(9, 18)
        .reduce((sum, score) => sum + score, 0);
      frontBackRanking.push({
        name: p.name,
        improvement: front - back,
        front,
        back,
        date: r.date,
        courseName: r.courseName,
  ...
  const frontBackImprovementRanking = frontBackRanking
  ...
  const handicapImproveRanking = [...playerRounds.entries()]
    .map(([name, list]) => {
      ...
      const pastHandicap = Math.ceil(
        avgOf(sorted.slice(0, handicapBasis).map((r) => r.diff)),
      );
      const recentHandicap = Math.ceil(
        avgOf(sorted.slice(-handicapBasis).map((r) => r.diff)),
      );
      return {
        name,
        improvement: pastHandicap - recentHandicap,
        pastHandicap,
        recentHandicap,
      };
    })
  ...
  const mostWinsText = topWinner
    ? formatWinners(
        winRanking.filter((r) => r.wins === topWinner.wins).map((r) => r.name),
        `${topWinner.wins}회`,
      )
    : "-";
  const lowestHandicapEntry = handicapRanking[0];
  const lowestHandiText = lowestHandicapEntry
    ? formatWinners(
        handicapRanking
          .filter((r) => r.handicap === lowestHandicapEntry.handicap)
          .map((r) => r.name),
        diffText(lowestHandicapEntry.handicap),
      )
    : "-";
  const topBirdie = birdieRanking[0];
  const topBirdieText =
    topBirdie && topBirdie.count > 0
      ? formatWinners(
          birdieRanking
            .filter((r) => r.count === topBirdie.count)
            .map((r) => r.name),
          `${topBirdie.count}개`,
        )
      : "-";
  const topSingleBirdie = singleBirdieRanking[0];
  const topSingleBirdieText = topSingleBirdie
    ? formatWinners(
        singleBirdieRanking
          .filter((r) => r.count === topSingleBirdie.count)
          .map((r) => r.name),
        `${topSingleBirdie.count}개`,
      )
    : "-";
  const topSinglePar = singleParRanking[0];
  const topSingleParText = topSinglePar
    ? formatWinners(
        singleParRanking
          .filter((r) => r.count === topSinglePar.count)
          .map((r) => r.name),
        `${topSinglePar.count}개`,
      )
    : "-";
  const topRoundsPlayed = [...playerRounds.entries()]
    .map(([name, list]) => ({ name, count: list.length }))
    .sort((a, b) => b.count - a.count)[0];
  const roundsPlayedText = topRoundsPlayed
    ? formatWinners(
        [...playerRounds.entries()]
          .filter(([, list]) => list.length === topRoundsPlayed.count)
          .map(([name]) => name),
        `${topRoundsPlayed.count}회`,
      )
    : "-";
  const topLowestScore = lowestScoreRanking[0];
  const lowestScoreText = topLowestScore
    ? formatWinners(
        lowestScoreRanking
          .filter((r) => r.total === topLowestScore.total)
          .map((r) => r.name),
        `${topLowestScore.total}타`,
      )
    : "-";
  const topHighestScore = highestScoreRanking[0];
  const highestScoreText = topHighestScore
    ? formatWinners(
        highestScoreRanking
          .filter((r) => r.total === topHighestScore.total)
          .map((r) => r.name),
        `${topHighestScore.total}타`,
      )
    : "-";
  const topFrontBack = frontBackImprovementRanking[0];
  const topFrontBackText = topFrontBack
    ? `${shortName(topFrontBack.name)} (${topFrontBack.improvement}타 개선)`
    : "-";
  const topAvgImprove = avgImproveRanking[0];
  const topAvgImproveText =
    topAvgImprove && topAvgImprove.improvement > 0
      ? `${shortName(topAvgImprove.name)} (${topAvgImprove.improvement}타 개선)`
      : "-";
  const topHandicapImprove = handicapImproveRanking[0];
  const topHandicapImproveText =
    topHandicapImprove && topHandicapImprove.improvement > 0
      ? `${shortName(topHandicapImprove.name)} (${topHandicapImprove.improvement}타 개선)`
      : "-";
  ...
  > = {
    ...
    roundsPlayed: {
      ...
      rows: [...playerRounds.entries()]
        ...
        .sort(
          (a, b) =>
            Number(b.value.replace("회", "")) -
            Number(a.value.replace("회", "")),
        ),
    },
    lowestScore: {
      title: "최저타",
      col: "스코어",
      rows: lowestScoreRanking.map((r) => ({
        name: shortName(r.name),
        value: `${r.total}타`,
        sub: `${r.date.slice(5)} ${r.courseName}`,
      })),
    ...
    highestScore: {
      title: "최고타",
      col: "스코어",
      rows: highestScoreRanking.map((r) => ({
        name: shortName(r.name),
        value: `${r.total}타`,
        sub: `${r.date.slice(5)} ${r.courseName}`,
      })),
    ...
    frontBack: {
      ...
      col: "개선",
      rows: frontBackImprovementRanking.map((r) => ({
        name: shortName(r.name),
        value: `${r.improvement}타`,
        sub: `${r.date.slice(5)} ${r.courseName} · 전반 ${r.front} / 후반 ${r.back}`,
      })),
    },
    avgImprove: {
      title: "최대 평균타 개선",
      col: "개선",
      rows: avgImproveRanking.map((r) => ({
        name: shortName(r.name),
        value: `${r.improvement}타`,
        sub: `과거7 ${r.pastAvg}타 → 최근3 ${r.recentAvg}타`,
      })),
    },
    handicapImprove: {
      title: "최대 핸디 개선",
      col: "개선",
      rows: handicapImproveRanking.map((r) => ({
        name: shortName(r.name),
        value: `${r.improvement}타`,
        sub: `초기 ${diffText(r.pastHandicap)} → 최근 ${diffText(r.recentHandicap)}`,
      })),
    },
  };
      ...
      items: [
        {
          icon: "🏆",
          label: "최저타",
          value: lowestScoreText,
          type: "lowestScore" as RankingType,
        },
        {
          icon: "📈",
          label: "최고타",
          value: highestScoreText,
          type: "highestScore" as RankingType,
        },
        {
          icon: "🐦",
          label: "버디왕 (전체)",
        ...
        },
        {
          icon: "◎",
          label: "파왕 (1경기)",
          value: topSingleParText,
          type: "singlePar" as RankingType,
        },
      ],
    },
    {
      ...
      items: [
        ...
        },
        {
          icon: "↘️",
          label: "전후반 개선",
          value: topFrontBackText,
          type: "frontBack" as RankingType,
        },
        {
          icon: "📊",
          label: "평균타 개선",
          value: topAvgImproveText,
          type: "avgImprove" as RankingType,
        },
        {
          icon: "🪄",
          label: "핸디 개선",
          value: topHandicapImproveText,
          type: "handicapImprove" as RankingType,
        },
      ],
    },
    {
      ...
      items: [
        {
          icon: "🗓️",
          label: "최다 라운드 참가",
          value: roundsPlayedText,
          type: "roundsPlayed" as RankingType,
        },
      ],
    },
  ];
            ...
            {config.rows.length === 0 ? (
              ...
              config.rows.map((row, i) => (
                <View key={i} style={s.tableRow}>
                  <Text style={[s.td, { flex: 0.6, textAlign: "center" }]}>
                    {i + 1}
                  </Text>
                  <View style={{ flex: 2.5 }}>
...
function TrendModal({
  ...
  const cx = (i: number) =>
    PAD.l + (data.length > 1 ? i / (data.length - 1) : 0.5) * chartW;
...
const s = StyleSheet.create({
  ...
  personalReportSection: { marginTop: 2, marginBottom: 14 },
  personalReportHeader: {
    flexDirection: "row",
    alignItems: "center",
    justifyContent: "space-between",
    marginBottom: 9,
  ...
  personalReportScroller: { paddingRight: 8, gap: 10 },
  personalReportCard: {
    width: 138,
    minHeight: 118,
    borderRadius: 18,
  ...
  personalReportIcon: { fontSize: 22, lineHeight: 26, marginBottom: 10 },
  personalReportCardTitle: {
    fontSize: 14,
    lineHeight: 18,
    fontWeight: "900",
    color: C.text,
